...
class KiCadLayerSpec {
    ...
    getLayerType(typeName) {
        const _typeName = typeName.replace(/"/g, "")
        switch (_typeName) {
            case "Cu":
                return "Copper"
            case "CrtYd":
...
class KiCadElement {
    ...
    findChild(obj, name, warn) {
        ...
        if (Array.isArray(name)) {
            ...
            console.log(
                "KiCadElement.findChild called with name argument that was neither string nor array."
            )
        }
        if (warn && child === null) {
    ...
    getExtents() {
        console.log(
            "KiCadElement.getExtents was not overridden in the child class."
        )
        return null
    }
    ...
    rescale(scale) {
        console.log(
            "KiCadElement.rescale was not overridden in the child class."
        )
    }
...
class KiCadLine extends KiCadElement {
    ...
    constructor(obj) {
        ...
        if (obj.type !== "fp_line") {
            console.log(
                "Error: attempted to construct KiCadLine from invalid input type."
            )
            return
        }
    ...
    draw(canvax, ctx, layerSide, layerType) {
        var currentLayer = this.layerSpec.layers.find(
            (layer) => layer.side == layerSide && layer.type == layerType
        )
...
class KiCadArc extends KiCadElement {
    ...
    constructor(obj) {
        ...
        if (obj.type !== "fp_arc") {
            console.log(
                "Error: attempted to construct KiCadArc from invalid input type."
            )
            return
        }
    ...
    getExtents() {
        ...
        let radius = Math.sqrt(
            Math.pow(this.from_x - this.to_x, 2) +
                Math.pow(this.from_y - this.to_y, 2)
        )
        const midpoint_x = (this.from_x + this.to_x) / 2
        const midpoint_y = (this.from_y + this.to_y) / 2
        return {
            min_x: Math.min(
                Math.min(this.from_x, this.to_x),
                midpoint_x - radius
            ),
            max_x: Math.max(
                Math.max(this.from_x, this.to_x),
                midpoint_x + radius
            ),
            min_y: Math.min(
                Math.min(this.from_y, this.to_y),
                midpoint_y - radius
            ),
            max_y: Math.max(
                Math.max(this.from_y, this.to_y),
                midpoint_y + radius
            ),
        }
    }
    ...
    draw(canvax, ctx, layerSide, layerType) {
        // todo
        var currentLayer =
            this.layerSpec.layers &&
            this.layerSpec.layers.find(
                (layer) => layer.side == layerSide && layer.type == layerType
            )
        if (currentLayer) {
...
class KiCadCircle extends KiCadElement {
    ...
    constructor(obj) {
        ...
        if (obj.type !== "fp_circle") {
            console.log(
                "Error: attempted to construct KiCadCircle from invalid input type."
            )
            return
        }
        ...
        if ((centerObj?.fields?.length ?? 0) < 2) {
            console.log(
                "fp_circle has insufficient values in 'center' specifier."
            )
            return
        }
        if ((endObj?.fields?.length ?? 0) < 2) {
        ...
        if ((widthObj?.fields?.length ?? 0) < 1) {
            console.log(
                "fp_circle has insufficient values in 'width' specifier."
            )
            return
        }
    ...
    getRadius() {
        return (
            Math.sqrt(
                Math.pow(this.pos_x - this.end_x, 2) +
                    Math.pow(this.pos_y - this.end_y, 2)
            ) / 2
        )
    }
    ...
    getExtents() {
        return {
            min_x: Math.min(
                this.from_x - this.getRadius(),
                this.end_x - this.getRadius()
            ),
            max_x: Math.max(
                this.from_x + this.getRadius(),
                this.end_x + this.getRadius()
            ...
            min_y: Math.min(
                this.from_y - this.getRadius(),
                this.end_y - this.getRadius()
            ),
            max_y: Math.max(
                this.from_y + this.getRadius(),
                this.end_y + this.getRadius()
...
class KiCadPad extends KiCadElement {
    ...
    constructor(obj) {
        ...
        if (obj.type !== "pad") {
            console.log(
                "Error: attempted to construct KiCadPad from invalid input type."
            )
            return
        }
        if (obj.fields.length < 3) {
            console.log(
                "Error: attempted to construct KiCadPad from an object with insufficient fields."
            )
            return
        }
        ...
        this.text = (obj.fields[0] || "").replace(/"/g, "")
        ...
        if (currentLayer) {
            if (this.type != "np_thru_hole") {
                if (this.type == "thru_hole" && layerType == "Copper") {
                    ctx.globalAlpha = 1
                    ctx.fillStyle =
                        KiCadDrawingSettings.LayerColours[layerSide]["PadTH"]
                } else {
                    ...
                    case "rect": {
                        ...
                        let rsize_y =
                            this.rotation == 90 || this.rotation == 270
                                ...
                                : this.size_y
                        ctx.rect(
                            this.pos_x - rsize_x / 2,
                            this.pos_y - rsize_y / 2,
                            rsize_x,
                            rsize_y
                        )
                        ctx.fill()
                    ...
                    case "roundrect": {
                        ...
                        let rsize_y =
                            this.rotation == 90 || this.rotation == 270
                                ...
                                : this.size_y
                        let r =
                            this.rounding * Math.min(this.size_x, this.size_y)
                        KiCadCanvasHelper.roundedRect(
                            ctx,
                            this.pos_x - rsize_x / 2,
                            this.pos_y - rsize_y / 2,
                            rsize_x,
            ...
            if (this.text) {
                ctx.fillStyle = "white"
                ctx.font = `${(this.size_x + this.size_y) / 2}px serif`
                // ctx.globalAlpha = 100
                ctx.fillText(
                    this.text,
                    this.pos_x - this.size_x / 4,
                    // this.pos_x + ,
                    this.pos_y + this.size_y / 3
                )
            }
        }
    }
...
function kicad_parse_syntax(code) {
    ...
    if (rootObject.children.length > 1) {
        console.log(
            "Warning: more than one root object found. Only returning the first."
        )
    }
...
function kicad_preview_canvas_handler(event, kicadFileUrl, boxSelector) {
    // check if the preview already exists
    if (document.getElementById("kicad_preview_canvas") !== null) {
        // console.log(
        //     "Not executing handler because kicad_preview_canvas already exists."
        // )
        return
    }
    ...
    // if (contribBox === null) {
    //     console.log(
    //         "Failed to find blob_contributors_box element to inject canvas preview after."
    //     )
    //     return
    // }
    ...
    //     if (headerBox == null) {
    //         console.log(
    //             "Failed to find header box element as a parent of the contributors box."
    //         )
    //         return
    //     }
    // } while (!headerBox.classList.contains("Box"))
    // inject preview box & canvas
    const headerBox = document.querySelector(boxSelector)
    headerBox.insertAdjacentHTML(
        "beforeend",
        '<div id="kicad_preview_container" class="Box d-flex flex-column flex-shrink-0 mb-3"><div class="Box-header Details js-details-container"></div><canvas id="kicad_preview_canvas"></canvas></div>'
    )

    // fetch plaintext
    // var rawUrl = "./ee.kicad_mod"
    ...
    fetch(kicadFileUrl, {
        ...
        .then(function (response) {
            if (response.status !== 200) {
                console.log(
                    "Failed to get raw code for footprint. Status: " +
                        response.status
                )
                return
            }
            ...
            response.text().then(function (code) {
                if (
                    document.getElementById("kicad_preview_container") === null
                ) {
                ...
                let scale = Math.min(
                    canvas.width / delta_x,
                    canvas.height / delta_y
                )
                ...
                for (
                    let x = max_x + canvas.width;
                    x > min_x - canvas.width;
                    x -= scale
                ) {
                    ctx.beginPath()
                ...
                for (
                    let y = max_y + canvas.height;
                    y > min_y - canvas.height;
                    y -= scale
                ) {
                    ctx.beginPath()
...
export const loadKicadFile = (filePath, boxSelector) => {
    window.addEventListener("load", (e) => {
        kicad_preview_canvas_handler(e, filePath, boxSelector)
    })

    kicad_preview_canvas_observer = new MutationObserver((mutationRecords) => {
        kicad_preview_canvas_handler(mutationRecords, filePath, boxSelector)
    })

    kicad_preview_canvas_observer.observe(document, {
        childList: true,
        subtree: true,
    })
}
